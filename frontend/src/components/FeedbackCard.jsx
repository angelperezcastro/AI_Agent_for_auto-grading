...
const LONG_FEEDBACK_CHARACTER_LIMIT = 900;
const LONG_FEEDBACK_LINE_LIMIT = 8;
...
function isLongFeedbackText(feedback) {
  const text = String(feedback || "");
  const lineCount = text.split(/\r?\n/).filter(Boolean).length;

  return (
    text.length > LONG_FEEDBACK_CHARACTER_LIMIT ||
    lineCount > LONG_FEEDBACK_LINE_LIMIT
  );
}
...
function FeedbackText({ feedback, mounted }) {
  ...
  const feedbackId = "ai-feedback-full-text";
  const isLong = isLongFeedbackText(feedback);
  const shouldCollapse = isLong && !expanded;

  return (
    <section
      className={[
        "mt-6 overflow-hidden rounded-3xl border border-cyan-100 bg-gradient-to-br from-cyan-50 via-white to-slate-50 shadow-sm transition",
      ...
      aria-labelledby="ai-feedback-heading"
    >
      <div className="border-b border-cyan-100/80 px-5 py-4">
        <div className="flex items-start gap-3">
          <div
            aria-hidden="true"
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-cyan-600 text-white shadow-sm shadow-cyan-200"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
          ...
          <div className="min-w-0">
            ...
            >
              Original evaluator comments
            </h4>

            <p className="mt-1 text-sm leading-6 text-slate-500">
              Read this feedback before continuing with the next deliverable.
              The text below is shown exactly as generated by the AI.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative px-5 py-5">
        <div
          id={feedbackId}
          className={[
            "relative text-[15px] leading-8 text-slate-700 sm:text-sm sm:leading-7",
            shouldCollapse ? "max-h-72 overflow-hidden" : "",
          ].join(" ")}
        >
        ...
        {shouldCollapse && (
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-x-5 bottom-0 h-24 bg-gradient-to-t from-slate-50 via-slate-50/95 to-transparent"
          />
        )}
      </div>

      {isLong && (
        <div className="border-t border-cyan-100/80 bg-white/70 px-5 py-4">
          <button
            type="button"
            aria-expanded={expanded}
            aria-controls={feedbackId}
            onClick={() => setExpanded((current) => !current)}
            className="inline-flex w-full items-center justify-center rounded-2xl border border-cyan-200 bg-white px-4 py-3 text-sm font-black text-cyan-800 transition hover:bg-cyan-50 focus:outline-none focus:ring-4 focus:ring-cyan-100 sm:w-auto"
          >
...
export default function FeedbackCard({ evaluation }) {
  ...
  return (
    ...
    >
      ...
      <FeedbackText feedback={evaluation.feedback} mounted={mounted} />
    </section>
  );
}